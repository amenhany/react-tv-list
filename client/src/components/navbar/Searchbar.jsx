...
export default function Searchbar({ decoration = false, delay = 0 }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="d-flex position-relative" id="searchForm" action="/search">
            ...
            <input value={searchText} onChange={handleChange} className={"form-control " + (decoration ? "search-input" : "me-1")} type="search" placeholder={decoration ? "Try searching something..." : "Search"} aria-label="Search" name="q" />
            { !decoration &&
